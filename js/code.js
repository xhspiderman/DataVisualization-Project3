...
//main function
function main(){
  var character_select = characters_DB().limit(50).get()
  var episodes = episodes_info_DB().limit(60).get()
  var properties = visualProperties(character_select,episodes)
  Plot(data_heatmap,properties)
}

function visualProperties(characters,episodes){
  var colLabel = [];
  var rowLabel = [];
  for (var i=0; i<episodes.length; i++)
  {
    colLabel.push('S:' + episodes[i].seasonNum + 'E:' + episodes[i].episodeNum)
  }
  for (var i=0; i<characters.length; i++)
  {
    rowLabel.push(characters[i].page)
  }

  var tableProperties = 
    {
      margin : { top: 150, right: 10, bottom: 50, left: 150 },
      cellSize : 12,
...
function Plot(data,properties) {
  //Initialize variables for the size of the svg object and parameters for creating the heat diagram
  var margin = properties.margin
  var cellSize=properties.cellSize;
  var col_number=properties.col_number;
  var row_number=properties.row_number;
  ...
  var colorBuckets = properties.color.colorBuckets;
  var colors = properties.color.colors;
  
  var hcrow = [49,11,30,4,18,6,12,20,19,33,32,26,44,35,38,3,23,41,22,10,2,15,16,36,8,25,29,7,27,34,48,31,45,43,14,9,39,1,37,47,42,21,40,5,28,46,50,17,24,13]; // change to gene name or probe id
  ...
  var rowLabels = svg.append("g")
      .selectAll(".rowLabelg")
      .data(properties.label.rowLabel)
      .enter()
      .append("text")
  ...
  var colLabels = svg.append("g")
      .selectAll(".colLabelg")
      .data(properties.label.colLabel)
      .enter()
      .append("text")
      .text(function (d) { return d; })
      .attr("x", 0)
  ...
  var heatMap = svg.append("g").attr("class","g3")
      ...
      .on("mouseover", function(d){
             ...
             d3.select("#tooltip")
               ...
               .select("#value")
               .text("lables:"+properties.label.rowLabel[d.row-1]+","+properties.label.colLabel[d.col-1]+"\ndata:"+d.value+"\nrow-col-idx:"+d.col+","+d.row+"\ncell-xy "+this.x.baseVal.value+", "+this.y.baseVal.value);  
             //Show the tooltip
